...
gsigs = 'https://github.com/liberty-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('liberty'):
        subprocess.check_call(['git', 'clone', args.url, 'liberty'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/liberty/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'liberty='+args.commit, '--url', 'liberty='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/liberty-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/liberty/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/liberty/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/liberty-project/liberty', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/liberty', exist_ok=True)
    os.chdir('builder/inputs/liberty')
    if args.pull:
